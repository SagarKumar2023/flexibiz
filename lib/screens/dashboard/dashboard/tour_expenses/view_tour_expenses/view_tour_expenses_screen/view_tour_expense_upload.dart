...
class ViewTourExpenseUpload extends StatefulWidget {
  final NewTourExpenseHiveModel item;

  const ViewTourExpenseUpload({super.key, required this.item});

  @override
  State<ViewTourExpenseUpload> createState() => _ViewTourExpenseUploadState();
}

class _ViewTourExpenseUploadState extends State<ViewTourExpenseUpload> {
  ...
  void _validateAndSubmit() {
    ...
    /// 2️ AT LEAST ONE MEDIA REQUIRED
    final bool hasImage = selectedImages.isNotEmpty;
    final bool hasVideo = selectedVideos.isNotEmpty;
    // final bool hasAudio = selectedAudio != null;

    if (!hasImage && !hasVideo) {
      SnackBarUtils.showWarning(
        'Warning',
        'Please select at least one media (Image, Video or Audio)',
      );
      return;
    }
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: Column(
          ...
          children: [
            TextWidget(
              text: widget.item.tourName.toUpperCase(),
              textColor: ColorUtils.white,
              fontSize: 15,
              fontWeight: FontWeight.w700,
            ),
            TextWidget(
              text: "Dt. ${DateFormatterUtils.formatDate(widget.item.tourDate)} Amount ${widget.item.tourAmount}",
